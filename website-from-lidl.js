...
async function main() {
    try {
        ...
        for (let i = 0; i < categories.products.length; i++) {
            ...
            const contentFilePath = `./data/websites/${category.slug}.json`;
            if (fs.existsSync(contentFilePath)) {
                console.log(`${i + 1}/${categories.products.length} - Already exists`);
                continue;
            }

            const keyword = category.productTitle;
            const description = category.productDescription;

            //console.log(`${i + 1}/${categories.products.length}`);

            // Générer le contenu de la page d'accueil
            const homepageContent = await generateHomepageContent(keyword, description);

            // Parser le contenu généré et mettre à jour content.json
            const lines = homepageContent.split('\n').map(line => line.trim()).filter(line => line);
            const site = {
                keyword: keyword,
                keywordPlurial: keyword.replace(/ /, 's ').replace(/ss/, 's'),
                slug: category.slug,
                source: category.productSource,
                shopName: lines[0],
                heroTitle: lines[1],
            ...
            const contentDir = './data/websites';
